...
- [ ] Extract the simulation - explicit state object + `stepGame(state, dtMs)` in its own module; `main.ts` keeps input, selection, render, DOM. **The checkpoint is that nothing changes** - no new behavior, no new numbers. Unblocks the harness and shrinks the project's biggest file.
- [ ] Balance harness - headless script that runs the waves against a declared tower layout at a fixed timestep and reports leaks, core HP, and Cycles per wave. Cheap because the sim is already deterministic: the only `Math.random()` in `src/` is in the cosmetic glitch particles. First tests in the project.
