...
- **Enemies**: Worm (fast/weak swarm unit), Trojan (armored/slow tank), Packet Sniffer (very fast, very low HP, large groups), Ransomware (splits into two weaker Encryptors on death), Beacon (keeps its own pace - the slow auras do nothing to it), Zero-Day (boss, high HP, immune to one tower type's effect). What each one asks of the player, and which tower answers it, is [Roster](./Roster.md).
